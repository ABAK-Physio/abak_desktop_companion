...
class AbakImportLauncher {
    ...
    try {
      ...
      for (final pickedFile in result.files) {
        try {
          ...
          processedFiles++;
          totalImportedResults += summary.importedResults;
          ...
          debugPrint('❌ Erreur import fichier ${pickedFile.name} : $e');
          debugPrint('$stack');
        }
      }
      ...
      final resolutionText = resolutionMessages.isEmpty
          ? ''
  ...
  static Future<Map<String, dynamic>> importArchiveFromPath(
    ...
    try {
      ...
      return {
        'status': 'needs_resolution',
        'message': 'Rattachement manuel requis.',
        'fileName': fileName,
        'filePath': filePath,
      };
    } catch (e, stack) {
...
class AbakImportLauncherResult {
  ...
  bool get hasImportedSomething {
    return importedResults > 0 || importedMetrics > 0;
  }
